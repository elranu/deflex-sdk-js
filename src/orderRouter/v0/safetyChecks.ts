...
export async function performSafetyChecks(algod: Algodv2,
    ...
    try {
        ...
        if (receivedAmountSum < (Number(expectedReceiveAmount) - 1)) {
            throw new Error(`Receives amount succeeds expected sent amount by ${Number(expectedReceiveAmount) - receivedAmountSum}!`)
        }
    } catch (error) {
        const parentError = error as Error;
        const err = new Error("Error performing safety checks: " + parentError.message);
